...
local function showLoader()
    ...
    loaderGui.Name = "BinxixLoader_Splash"; loaderGui.ResetOnSpawn = false
    loaderGui.IgnoreGuiInset = true; loaderGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    loaderGui.Parent = player:WaitForChild("PlayerGui")

    -- Dim background
    local bg = Instance.new("Frame")
    bg.Size = UDim2.new(1,0,1,0); bg.BackgroundColor3 = Color3.fromRGB(0,0,0)
    bg.BackgroundTransparency = 0.18; bg.BorderSizePixel = 0; bg.ZIndex = 200; bg.Parent = loaderGui

    -- Card
    local card = Instance.new("Frame")
    card.Size = UDim2.new(0,310,0,270); card.AnchorPoint = Vector2.new(0.5,0.5)
    card.Position = UDim2.new(0.5,0,0.5,0); card.BackgroundColor3 = Color3.fromRGB(13,13,15)
    card.BorderSizePixel = 0; card.ZIndex = 201; card.Parent = loaderGui
    Instance.new("UICorner", card).CornerRadius = UDim.new(0,12)
    local cardStroke = Instance.new("UIStroke", card)
    cardStroke.Color = Color3.fromRGB(50,50,58); cardStroke.Thickness = 1.2

    local function mkLbl(parent, txt, pos, sz, col, tSize, font, align, zIdx)
        local l = Instance.new("TextLabel")
        l.Text = txt; l.Size = sz; l.Position = pos; l.BackgroundTransparency = 1
        l.TextColor3 = col; l.TextSize = tSize; l.Font = font or Enum.Font.SourceSans
        l.TextXAlignment = align or Enum.TextXAlignment.Center
        l.ZIndex = zIdx or 202; l.Parent = parent; return l
    end

    -- Logo circle
    local logoRing = Instance.new("Frame")
    logoRing.Size = UDim2.new(0,64,0,64); logoRing.AnchorPoint = Vector2.new(0.5,0)
    logoRing.Position = UDim2.new(0.5,0,0,22); logoRing.BackgroundColor3 = Color3.fromRGB(22,22,27)
    logoRing.BorderSizePixel = 0; logoRing.ZIndex = 202; logoRing.Parent = card
    Instance.new("UICorner", logoRing).CornerRadius = UDim.new(1,0)
    local logoRingStroke = Instance.new("UIStroke", logoRing)
    logoRingStroke.Color = Color3.fromRGB(60,55,70); logoRingStroke.Thickness = 1.5

    -- Logo letter
    local logoLetter = mkLbl(logoRing, "B", UDim2.new(0,0,0,0), UDim2.new(1,0,1,0),
        Color3.fromRGB(200,140,255), 28, Enum.Font.GothamBold, Enum.TextXAlignment.Center, 203)
    logoLetter.TextYAlignment = Enum.TextYAlignment.Center

    -- Title row: "binxix hub  •  stable"
    local titleLbl = mkLbl(card, "binxix hub", UDim2.new(0,0,0,96), UDim2.new(0.54,0,0,18),
        Color3.fromRGB(220,220,228), 15, Enum.Font.GothamBold, Enum.TextXAlignment.Right, 202)
    local dotLbl = mkLbl(card, "•", UDim2.new(0.55,0,0,96), UDim2.new(0.06,0,0,18),
        Color3.fromRGB(90,90,100), 14, Enum.Font.Gotham, Enum.TextXAlignment.Center, 202)
    local stableLbl = mkLbl(card, "Stable", UDim2.new(0.62,0,0,96), UDim2.new(0.36,0,0,18),
        Color3.fromRGB(90,210,130), 14, Enum.Font.GothamBold, Enum.TextXAlignment.Left, 202)

    -- guns.lol row
    local gunsRow = Instance.new("Frame")
    gunsRow.Size = UDim2.new(1,-24,0,44); gunsRow.Position = UDim2.new(0,12,0,124)
    gunsRow.BackgroundColor3 = Color3.fromRGB(60,48,92); gunsRow.BorderSizePixel = 0; gunsRow.ZIndex = 202; gunsRow.Parent = card
    Instance.new("UICorner", gunsRow).CornerRadius = UDim.new(0,8)
    local gunsStroke = Instance.new("UIStroke", gunsRow)
    gunsStroke.Color = Color3.fromRGB(110,80,180); gunsStroke.Thickness = 1.2
    mkLbl(gunsRow, "🔫", UDim2.new(0,10,0,0), UDim2.new(0,28,1,0),
        Color3.fromRGB(255,255,255), 16, Enum.Font.SourceSans, Enum.TextXAlignment.Center, 203)
    mkLbl(gunsRow, "guns.lol", UDim2.new(0,40,0,0), UDim2.new(0,100,1,0),
        Color3.fromRGB(220,215,240), 14, Enum.Font.GothamBold, Enum.TextXAlignment.Left, 203)
    mkLbl(gunsRow, "@binxix", UDim2.new(0,148,0,0), UDim2.new(1,-158,1,0),
        Color3.fromRGB(160,120,255), 13, Enum.Font.Gotham, Enum.TextXAlignment.Right, 203)

    -- Bottom row: Discord + GitHub
    local discordBtn = Instance.new("Frame")
    discordBtn.Size = UDim2.new(0.5,-16,0,38); discordBtn.Position = UDim2.new(0,12,0,180)
    discordBtn.BackgroundColor3 = Color3.fromRGB(28,24,42); discordBtn.BorderSizePixel = 0; discordBtn.ZIndex = 202; discordBtn.Parent = card
    Instance.new("UICorner", discordBtn).CornerRadius = UDim.new(0,7)
    local discordStroke = Instance.new("UIStroke", discordBtn)
    discordStroke.Color = Color3.fromRGB(80,60,120); discordStroke.Thickness = 1
    mkLbl(discordBtn, "🔗 Discord", UDim2.new(0,8,0,0), UDim2.new(1,-8,0.6,0),
        Color3.fromRGB(180,160,230), 13, Enum.Font.GothamBold, Enum.TextXAlignment.Left, 203)
    mkLbl(discordBtn, "@binxix", UDim2.new(0,8,0.52,0), UDim2.new(1,-8,0.48,0),
        Color3.fromRGB(130,110,170), 11, Enum.Font.Gotham, Enum.TextXAlignment.Left, 203)

    local githubBtn = Instance.new("Frame")
    githubBtn.Size = UDim2.new(0.5,-16,0,38); githubBtn.Position = UDim2.new(0.5,4,0,180)
    githubBtn.BackgroundColor3 = Color3.fromRGB(24,26,30); githubBtn.BorderSizePixel = 0; githubBtn.ZIndex = 202; githubBtn.Parent = card
    Instance.new("UICorner", githubBtn).CornerRadius = UDim.new(0,7)
    local githubStroke = Instance.new("UIStroke", githubBtn)
    githubStroke.Color = Color3.fromRGB(60,64,72); githubStroke.Thickness = 1
    mkLbl(githubBtn, "⬡ GitHub", UDim2.new(0,8,0,0), UDim2.new(1,-8,0.6,0),
        Color3.fromRGB(200,200,210), 13, Enum.Font.GothamBold, Enum.TextXAlignment.Left, 203)
    mkLbl(githubBtn, "@binx-ux", UDim2.new(0,8,0.52,0), UDim2.new(1,-8,0.48,0),
        Color3.fromRGB(130,134,145), 11, Enum.Font.Gotham, Enum.TextXAlignment.Left, 203)

    -- checkmark + timer
    local checkLbl = mkLbl(card, "✓", UDim2.new(0,0,0,228), UDim2.new(1,0,0,18),
        Color3.fromRGB(80,220,120), 16, Enum.Font.GothamBold, Enum.TextXAlignment.Center, 202)
    checkLbl.TextYAlignment = Enum.TextYAlignment.Center

    local timerLbl = mkLbl(card, "V354  |  Time till Load: 3.0", UDim2.new(0,10,1,-20), UDim2.new(1,-20,0,14),
        Color3.fromRGB(70,70,82), 10, Enum.Font.Gotham, Enum.TextXAlignment.Left, 202)

    -- Countdown — blocks until done, then destroys loader
    local elapsed = 0
    local totalTime = 3.0
    while elapsed < totalTime do
        ...
        timerLbl.Text = "V354  |  Time till Load: " .. string.format("%.1f", remaining)
    end
...
local function createGUI()
    ...
    local function buildESPTab(page)
        if gameConfig.espEnabled then
            ...
            -- ===== ESP OVERRIDE MODE (MiscGun-X and other unsupported games) =====
            local espOverrideEnabled = false

            local buildOverrideWarning  -- forward declare
            local function buildFullESPControls()
                -- Clear override warning, show full controls
                for _, child in ipairs(page:GetChildren()) do child:Destroy() end

                -- Slim override-active banner at top
                local activeBanner = Instance.new("Frame"); activeBanner.Size = UDim2.new(1,-16,0,28); activeBanner.Position = UDim2.new(0,8,0,4)
                activeBanner.BackgroundColor3 = Color3.fromRGB(30,50,30); activeBanner.BorderSizePixel = 1; activeBanner.BorderColor3 = Color3.fromRGB(60,160,60); activeBanner.Parent = page
                Instance.new("UICorner", activeBanner).CornerRadius = UDim.new(0,5)
                local activeLbl = Instance.new("TextLabel"); activeLbl.Size = UDim2.new(0.8,0,1,0); activeLbl.Position = UDim2.new(0,10,0,0)
                activeLbl.BackgroundTransparency = 1; activeLbl.Text = "✓ ESP Override active — " .. currentGameData.name
                activeLbl.TextColor3 = Color3.fromRGB(90,220,100); activeLbl.TextSize = 11; activeLbl.Font = Enum.Font.GothamBold
                activeLbl.TextXAlignment = Enum.TextXAlignment.Left; activeLbl.Parent = activeBanner
                local disableBtn = Instance.new("TextButton"); disableBtn.Size = UDim2.new(0,60,0,20); disableBtn.Position = UDim2.new(1,-68,0,4)
                disableBtn.BackgroundColor3 = Color3.fromRGB(60,30,30); disableBtn.BorderSizePixel = 1; disableBtn.BorderColor3 = Color3.fromRGB(160,50,50)
                disableBtn.Text = "Disable"; disableBtn.TextColor3 = Color3.fromRGB(240,100,100); disableBtn.TextSize = 11
                disableBtn.Font = Enum.Font.GothamBold; disableBtn.Parent = activeBanner
                Instance.new("UICorner", disableBtn).CornerRadius = UDim.new(0,4)
                disableBtn.MouseButton1Click:Connect(function()
                    espOverrideEnabled = false; gameConfig.espEnabled = false
                ...
                -- Full ESP controls (same as supported games, offset by yOff)
                createSectionHeader(page, "ESP Properties", 0, yOff)
                createCheckbox(page, "Enabled", 0, yOff+22, false, function(e) Settings.ESP.Enabled = e; sendNotification("ESP", e and "On" or "Off", 2) end)
                createDropdown(page, "Filter", 0, yOff+44, {"Enemies","Team","All","All (No Team Check)"}, "Enemies", function(v) Settings.ESP.FilterMode = v end)
                createCheckbox(page, "Display Distance", 0, yOff+84, true, function(e) Settings.ESP.DistanceEnabled = e end)
                createCheckbox(page, "Display Name", 0, yOff+106, true, function(e) Settings.ESP.NameEnabled = e end)
                createCheckbox(page, "Display Health", 0, yOff+128, true, function(e) Settings.ESP.HealthEnabled = e end)
                createCheckbox(page, "Display Box", 0, yOff+150, true, function(e) Settings.ESP.BoxEnabled = e end)
                createCheckbox(page, "Chams (Wallhack Glow)", 0, yOff+172, false, function(e) Settings.ESP.ChamsEnabled = e; sendNotification("Chams", e and "On" or "Off", 2) end)
                createSlider(page, "Chams Fill", 0, yOff+196, 0, 100, 50, function(v) Settings.ESP.ChamsFillTransparency = v/100 end)
                createCheckbox(page, "Rainbow Outline", 0, yOff+232, false, function(e) Settings.ESP.RainbowOutline = e end)
                createCheckbox(page, "Rainbow Color", 0, yOff+254, false, function(e) Settings.ESP.RainbowColor = e end)
                createCheckbox(page, "Outline", 0, yOff+276, true, function(e) Settings.ESP.OutlineEnabled = e end)
                createSlider(page, "Font Size", 0, yOff+300, 10, 24, 13, function(v) Settings.ESP.FontSize = v end)

                createSectionHeader(page, "Tracer", 240, yOff)
                createCheckbox(page, "Tracer Enabled", 240, yOff+22, true, function(e) Settings.ESP.TracerEnabled = e end)
                createCheckbox(page, "Rainbow Color", 240, yOff+44, false, function(e) Settings.ESP.TracerRainbowColor = e end)
                createDropdown(page, "Position", 240, yOff+70, {"Bottom","Center","Mouse"}, "Bottom", function(v) Settings.ESP.TracerOrigin = v end)
                createSlider(page, "Thickness", 240, yOff+110, 1, 5, 1, function(v) Settings.ESP.TracerThickness = v end)
                createSectionHeader(page, "Skeleton ESP", 240, yOff+150)
                createCheckbox(page, "Skeleton Enabled", 240, yOff+172, false, function(e) Settings.ESP.SkeletonEnabled = e end)
                createSlider(page, "Skeleton Thickness", 240, yOff+194, 1, 5, 1, function(v) Settings.ESP.SkeletonThickness = v end)
                createSectionHeader(page, "Offscreen Arrows", 240, yOff+234)
                createCheckbox(page, "Arrows Enabled", 240, yOff+256, false, function(e) Settings.ESP.OffscreenArrows = e end)
                createSlider(page, "Arrow Size", 240, yOff+278, 10, 40, 20, function(v) Settings.ESP.ArrowSize = v end)
                createSlider(page, "Arrow Distance", 240, yOff+314, 100, 1000, 500, function(v) Settings.ESP.ArrowDistance = v end)

                page.CanvasSize = UDim2.new(0,0,0,yOff+370)
            end

            buildOverrideWarning = function()
                -- Clear existing page children
                for _, child in ipairs(page:GetChildren()) do child:Destroy() end

                local warnBox = Instance.new("Frame"); warnBox.Size = UDim2.new(1,-16,0,120); warnBox.Position = UDim2.new(0,8,0,10)
                warnBox.BackgroundColor3 = Theme.BackgroundLight; warnBox.BorderSizePixel = 1; warnBox.BorderColor3 = Color3.fromRGB(200,140,40); warnBox.Parent = page

                local warnIcon = Instance.new("TextLabel"); warnIcon.Size = UDim2.new(0,24,0,24); warnIcon.Position = UDim2.new(0,10,0,10)
                warnIcon.BackgroundTransparency = 1; warnIcon.Text = "⚠"; warnIcon.TextColor3 = Color3.fromRGB(255,180,50)
                warnIcon.TextSize = 18; warnIcon.Font = Enum.Font.GothamBold; warnIcon.Parent = warnBox

                local warnTitle = Instance.new("TextLabel"); warnTitle.Size = UDim2.new(1,-44,0,24); warnTitle.Position = UDim2.new(0,38,0,10)
                warnTitle.BackgroundTransparency = 1; warnTitle.Text = "ESP — " .. currentGameData.name
                warnTitle.TextColor3 = Color3.fromRGB(255,180,50); warnTitle.TextSize = 13; warnTitle.Font = Enum.Font.GothamBold
                warnTitle.TextXAlignment = Enum.TextXAlignment.Left; warnTitle.Parent = warnBox

                local warnSub = Instance.new("TextLabel"); warnSub.Size = UDim2.new(1,-20,0,16); warnSub.Position = UDim2.new(0,10,0,36)
                warnSub.BackgroundTransparency = 1; warnSub.Text = "ESP is not officially supported for this game."
                warnSub.TextColor3 = Color3.fromRGB(180,165,165); warnSub.TextSize = 11; warnSub.Font = Enum.Font.SourceSansItalic
                warnSub.TextXAlignment = Enum.TextXAlignment.Left; warnSub.Parent = warnBox

                local overrideToggle = Instance.new("TextButton"); overrideToggle.Size = UDim2.new(1,-20,0,34); overrideToggle.Position = UDim2.new(0,10,0,62)
                overrideToggle.BackgroundColor3 = Color3.fromRGB(50,38,22); overrideToggle.BorderSizePixel = 1
                overrideToggle.BorderColor3 = Color3.fromRGB(180,120,30)
                overrideToggle.Text = "[ Enable ESP Override ]  (use at your own risk)"
                overrideToggle.TextColor3 = Color3.fromRGB(255,180,60); overrideToggle.TextSize = 12
                overrideToggle.Font = Enum.Font.GothamBold; overrideToggle.Parent = warnBox
                Instance.new("UICorner", overrideToggle).CornerRadius = UDim.new(0,5)

                overrideToggle.MouseButton1Click:Connect(function()
                    espOverrideEnabled = true
                    gameConfig.espEnabled = true
                    sendNotification("ESP Override", "Force-enabled for " .. currentGameData.name, 3)
                    buildFullESPControls()
                end)

                page.CanvasSize = UDim2.new(0,0,0,160)
            end
